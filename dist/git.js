...
function init(action) {
  return __awaiter(this, void 0, void 0, function* () {
    try {
      ...
      try {
        yield (0,
        execute_1.execute)(`git config --global --add safe.directory "${action.workspace}"`, action.workspace, action.silent);
      } catch (_a) {
        (0, core_1.info)("Unable to set workspace as a safe directory…");
      }
      yield (0,
      execute_1.execute)(`git config user.name "${action.name}"`, action.workspace, action.silent);
      yield (0,
      execute_1.execute)(`git config user.email "${action.email}"`, action.workspace, action.silent);
      yield (0,
      execute_1.execute)(`git config core.ignorecase false`, action.workspace, action.silent);
      try {
      ...
      try {
        yield (0,
        execute_1.execute)(`git remote rm origin`, action.workspace, action.silent);
      ...
      yield (0,
      execute_1.execute)(`git remote add origin ${action.repositoryPath}`, action.workspace, action.silent);
      (0, core_1.info)("Git configured… 🔧");
    } catch (error) {
      throw new Error(
        `There was an error initializing the repository: ${(0,
        util_1.suppressSensitiveInformation)(
          (0, util_1.extractErrorMessage)(error),
          action
        )} ❌`
      );
    }
...
function deploy(action) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    try {
      ...
      const branchExists =
        action.isTest & constants_1.TestFlag.HAS_REMOTE_BRANCH ||
        Boolean(
          (yield (0, execute_1.execute)(
      ...
      yield (0,
      worktree_1.generateWorktree)(action, temporaryDeploymentDirectory, branchExists);
      /* Relaxes permissions of folder due to be deployed so rsync can write to/from it. */
      try {
        yield (0,
        execute_1.execute)(`chmod -R +rw ${action.folderPath}`, action.workspace, action.silent);
      } catch (_a) {
      ...
      const hasFilesToCommit =
        action.isTest & constants_1.TestFlag.HAS_CHANGED_FILES ||
        Boolean(
          (yield (0, execute_1.execute)(
            checkGitStatus,
            `${action.workspace}/${temporaryDeploymentDirectory}`,
      ...
      // Commits to GitHub.
      yield (0,
      execute_1.execute)(`git add --all .`, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent);
      yield (0,
      execute_1.execute)(`git checkout -b ${temporaryDeploymentBranch}`, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent);
      yield (0,
      execute_1.execute)(`git commit -m "${commitMessage}" --quiet --no-verify`, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent);
      if (action.dryRun) {
      ...
      if (action.force) {
        ...
        do {
          ...
          (0, core_1.info)(
            `Pushing changes… (attempt ${attempt} of ${ATTEMPT_LIMIT})`
          );
          const pushResult = yield (0, execute_1.execute)(
          ...
          if (rejected) (0, core_1.info)("Updates were rejected");
      ...
      throw new Error(
        `The deploy step encountered an error: ${(0,
        util_1.suppressSensitiveInformation)(
          (0, util_1.extractErrorMessage)(error),
          action
        )} ❌`
      ...
      execute_1.execute)(`git checkout -B ${temporaryDeploymentBranch}`, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent);
      yield (0,
      execute_1.execute)(`chmod -R +rw ${temporaryDeploymentDirectory}`, action.workspace, action.silent);
      yield (0,
      execute_1.execute)(`git worktree remove ${temporaryDeploymentDirectory} --force`, action.workspace, action.silent);
